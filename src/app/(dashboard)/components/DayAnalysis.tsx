          ...
          <div className="px-4 pb-8 space-y-5">
            ...
            {analysis && !loading && (
              <>
                ...
                {/* Next best action — priority callout */}
                {analysis.next_best_action && (
                  <div className="flex gap-3 items-start p-3 rounded-xl bg-primary/8 border border-primary/20">
                    <Zap className="w-4 h-4 shrink-0 mt-0.5 text-primary" />
                    <div>
                      <span className="text-xs font-semibold uppercase tracking-wide text-primary">
                        Focus on {analysis.next_best_action.nutrient}
                      </span>
                      <p className="text-sm mt-0.5 leading-relaxed">{analysis.next_best_action.label}</p>
                    </div>
                  </div>
                )}

                {/* Body state */}
                <div className="p-3 rounded-xl bg-muted/50 text-sm text-muted-foreground leading-relaxed">
                  {analysis.body_state}
                </div>
